...
var path = require('path'),
    ...
    etag = require('./etag'),
    optsParser = require('./opts');

var StaticServer = module.exports = function (dir, options) {
  ...
  return function middleware (req, res, next) {
    ...
    if(serverHeader !== false) {
      // Set common headers.
      res.setHeader('server', 'StaticServer-'+version);
    }
...
StaticServer.version = version;
StaticServer.showDir = showDir;
...
// See: https://github.com/jesusabdullah/node-StaticServer/issues/109
...
if (!module.parent) {
  ...
  if (opts.help || opts.h) {
    var u = console.error;
    u('usage: ss [dir] {options} --port PORT');
    u('see https://npm.im/ss for more docs');
    return;
  }

  http.createServer(StaticServer(dir, opts))
    .listen(port, function () {
      console.log('ss(static-server) serving ' + dir + ' at http://0.0.0.0:' + port);
    });
}